  ...
  return (
    <div className="bg-white shadow-lg rounded-lg p-0 lg:p-8 pb-12 mb-8">
      ...
      <h1 className="transition duration-200 text-center mb-8 cursor-pointer hover:text-pink-600 text-3xl font-semibold ">
        <Link href={`/post/${post.slub}`}>{post.title}</Link>
      </h1>
      ...
      <div className="text-center">
        <Link href={`/post/${post.slug}`}>
          <span className="transition duration-500 transform hover:-translate-y-1 inline-block bg-pink-600 text-lg font-medium rounded-full text-white px-4 py-2 cursor-pointer">
            Continue reading
          </span>
        </Link>
      </div>
